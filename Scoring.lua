...
function LS:GetRecommendations()
  ...
  if self.db.goals.PETS and self.GetPetRecommendations then
    for _, pet in ipairs(self:GetPetRecommendations()) do
      consider(pet, pet.score)
    end
  end
...
function LS:ActivityHorizon(activity)
  ...
  if cat == "Great Vault" or cat == "PvP" or id == "delve" or id == "prey"
      ...
      or id:sub(1, 14) == "housing_quest_" or id:sub(1, 12) == "pets_weekly_"
      or id:sub(1, 6) == "vault_" then
    return "WEEKLY"
  end
  if id:sub(1, 10) == "kp_weekly_" or id:sub(1, 10) == "kp_gather_" then
    return "WEEKLY"
  end
  if cat == "Mounts" or cat == "Reputation" or cat == "Gold" or cat == "Housing"
      or id == "pets_training" then
    return "LONG"
  end
